...
Далее настроим шлюз (вируальную машину с внешним ip адресом `195.19.36.**`) таким образом, чтобы соединение с портом 2022 на интерфейсе ens160 перенаправлялось через интерфейс ens192 на порт 22 по сетевому адресу 192.168.1.2.
...
Далее разршим настройку iptables из файла /etc/iptable.restore при старте системы. Для этого создадим сервис:
...
Проверьте, что форвардинг портов работает:
